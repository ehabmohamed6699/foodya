...
const UpdateRecipe = ({recipe:recipeToUpdate, fetchRecipe}:{recipe:{[key: string]:any} ,fetchRecipe: () => void}) => {
    ...
    const [previousImage, setPreviousImage] = useState<string>(recipeToUpdate?.image)
    const [instruction, setInstruction] = useState<string>("")
    const [recipe, setRecipe] = useState<{[key:string]:any}>({...recipeToUpdate, ingredients: recipeToUpdate.ingredients.join(", ")})
    ...
    const updateRecipe = async () => {
        setIsLoading(true)
        const updatedRecipe = {...recipe, ingredients:recipe.ingredients.split(",").map((item:string) => item.trim())}
        try{
            for (const key in updatedRecipe) {
                if (key !== 'servingSize') {
                  const value = (updatedRecipe as {[key:string]:any})[key];
                  if (
            ...
            const res = await fetch(`/api/recipes?id=${(updatedRecipe as any)?._id}`, {
                method: 'PATCH',
                headers: {
                  'Content-Type': 'application/json',
    ...
    if(!(session?.user as any)?.token  || (session?.user as any)?.id !== recipeToUpdate?.user?._id){
        return(<></>)
    }
  return (
        
        <Modal content={<MdOutlineEdit />} buttonStyle="bg-[#FB6D48] text-white text-xl p-4 rounded-full fixed bottom-10 right-10 md:right-32" onClick={()=>{}} isOpen={open} setIsOpen={setOpen}>
            <h1 className="text-3xl font-bold mb-6 text-[#FB6D48]">Edit recipe</h1>
            <form onSubmit={(e)=>{
                e.preventDefault()
                updateRecipe()
                ...
                <button type="submit" disabled={isLoading} className='bg-[#FB6D48] text-white w-48 h-11 flex items-center justify-center rounded-full'>{isLoading?<span className="loading loading-spinner loading-md"></span>:"Edit recipe"}</button>
            </form>
